...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
      ...
      //by zhujianbo add onCloseBefore 事件
        onCloseBefore:null
    };
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            $(".l-dialog-tc .l-dialog-close", g.dialog).click(function ()
            {
            	//by zhujianbo 
            	var flag=true;
            	if(g.options.onCloseBefore){
            		g.options.onCloseBefore(g);
            	}else{
            		if (p.isHidden)
                        g.hide();
                    else
        ...
        _updateBtnsWidth: function ()
        {
            ...
            g.dialog.winbtns.width(25 * btnscount);
        },
    ...
    $.ligerDialog.waittingProcess = function (title,content)
    {
        title = title || $.ligerDefaults.Dialog.waittingMessage;
        return $.ligerDialog.open({ cls: 'l-dialog-waittingProcessdialog',type: 'none',title:title, content: content, allowClose: false });
    };
    $.ligerDialog.closeWaittingProcess = function ()
    {
        var dialogs = l.find(l.controls.Dialog);
        for (var i in dialogs)
        {
            var d = dialogs[i];
            if (d.dialog.hasClass("l-dialog-waittingProcessdialog"))
                d.close();
        }
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Filter.operators['combobox'] = 
    ["equal", "notequal"];
    ...
    $.ligerDefaults.Filter.editors['combobox'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            //alert("all:"+JSON.stringify(field.editor.data));
            var options = {
                //data: field.data,//by malq 2012-12-15
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _bulidGroupTableHtml: function (altering, allowDelete)
        {
            ...
            //tableHtmlArr.push('<option value="or">' + p.strings['or'] + '</option>');//by malq 2012-09-05
            tableHtmlArr.push('</select>');

            //add group
            //tableHtmlArr.push('<input type="button" value="' + p.strings['addgroup'] + '" class="addgroup">');//by malq 2012-09-05
            //add rule
        ...
        _bulidRuleRowHtml: function (fields)
        {
            ...
            rowHtmlArr.push('<select class="opsel" style="width:80px;">');
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                      <div id='grid_header1' class='l-grid-header l-grid-header1'>");
