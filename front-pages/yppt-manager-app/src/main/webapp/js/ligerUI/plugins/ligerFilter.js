...
(function ($)
{
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _bulidGroupTableHtml: function (altering, allowDelete)
        {
            ...
            //tableHtmlArr.push('<option value="or">' + p.strings['or'] + '</option>');//by malq 2012-06-16
            tableHtmlArr.push('</select>');

            //alert(33);
            //add group
            //tableHtmlArr.push('<input type="button" value="' + p.strings['addgroup'] + '" class="addgroup">');//by malq 2012-06-16
            //add rule
        ...
        _bulidRuleRowHtml: function (fields)
        {
            ...
            {
                var field = fields[i];
                if(field.name){// by malq 2012-02-07
