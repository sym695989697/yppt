...
(function() {
	...
	var Invoicecommonview = function() {
		this.init();
	};
	
	Invoicecommonview.prototype = {
		config : {
		...
		initAddOrEdit:function(){
			...
			JAjax(that.config.queryObjectUrl,{'model.id':id}, 'json', function(data){
				if(data){
    			    ...
    			    $("#invoiceName").text(data.dataObject.invoiceName == null ? null : data.dataObject.invoiceName);
    			    $("#userRegPhone").text(data.dataObject.userRegPhone == null ? null : data.dataObject.userRegPhone);
    			    $("#userName").text(data.dataObject.userName == null ? null : data.dataObject.userName);
...
//        			//开票结果
	            ...
	            submitHandler: function(){
	            	...
	            	$.ligerDialog.confirm('确定对记录进行审核或不通过处理吗？', function(yes) {
	   				 if(yes){
	   						...
	   						params['model.invoiceMoney']=$("#invoiceMoney").text();
	   						...
	   						params['model.expressOrderNo']=$("#actualInvoiceMoney").text();
	...
	$(document).ready(function() {
		window.invoicecommonview = new Invoicecommonview();
	});
})();