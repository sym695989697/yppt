(function() {
	...
	ImportRebateManager.prototype = {
		config : {
			...
			importUrl:root+'/reconciliation/importReconciliationFile.action'
		},
		cache:{
			processKey:'reconciliation_sy_import_process_key'
		},
		init:function(){
			var that=this;
			//模版下载
			$("#downLoadModel").attr("href",(root+"/reconciliation/downLoadModel.action"));
	  		//ajaxFrom
	  		var options = {
		...
		checkProgress:function(){
			...
			JAjax(that.config.getProcessUrl,{"processKey":that.cache.processKey}, 'json', function(data) {
				if(isNaN(data)){
					...
					parent.window.reconciliation.alert_message(data);
