...
ReconciliationSh.prototype = {
	...
	importReconciliation : function() {
		var title = '导入账单';
		var url = 'reconciliation-sh-import.jsp';
		window.reconciliationSh.dialog = MALQ_UI.open_button(url, 100, 500, window.reconciliationSh.importSuccess, window.reconciliationSh.cancel, title);
	},
	importSuccess : function(item, dialog) {
		window.reconciliationSh.dialog.frame.window.importRebateManager.f_save();
	},
	cancel : function(item, dialog) {
		window.reconciliationSh.dialog.close();
	},
	// 初始化列表框
	...
	loadGird : function() {
		...
		// 生成表格上面的按钮菜单///////////////////////////////
		var myitems = [];
		var funItems = FUNCTION_UTIL.getPageFunction(window.frameElement.id, [ 'import-reconciliation-sh' ]);
		$(funItems).each(function() {
			...
			var item = {};
			var click = null;
			if ('import-reconciliation-sh' == this.permId) {
		...
		var columns = [  {
			...
			align : 'left',
			render : function(row) {
				if(row.openCardOfficeCode){
					return CODE_UTIL.getCodeName(row.cardType,row.openCardOfficeCode);
				}else{
					return "";
				}
			}
			...
			render : function(row) {
				...
				if (productPrice != null && productPrice != "") {
					return (parseFloat(productPrice) / 100).toFixed(2);
				} else {
					return "0";
				}
		...
		options['parms'] = [{name:'requestParam.equal.cardType',value:'CARD-TYPE-02'}]; // 设置默认进来的请求参数
	...
	search : function() {
		...
		parms['requestParam.equal.cardType']='CARD-TYPE-02';
	...
	refreshGrid : function() {
		var that = window.reconciliationSh;
		that.grid.loadData(true);
	},
	alert_message : function(mes) {
		...
		var options = {btnOptions:[{ text: '确定', onclick:window.reconciliationSh.cancel  }
	        		              ]
	        						};
...
$(function() {
	var reconciliationSh = new ReconciliationSh();
	window.reconciliationSh = reconciliationSh;
	reconciliationSh.init();
});