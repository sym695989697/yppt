...
(function() {
	...
	MemberTradeList.prototype = {
		config : {
			queryGridUrl : root + '/mainCardTrade/queryListPage.action',
			outputUrl : root + '/mainCardTeade/outputMemberTradeList.action',
			subHeight : subHeight,
			subWidth : subWidth
		},
		cache : {
		...
		initMsg : function() {
			...
			CODE_UTIL.getSelectByCodeType($('#org'), 'CARD_ORG', 180, 80);
			//卡类型初始化
		...
		loadGird : function() {
			...
			$('#memberMainTradeList').ligerGrid(options);
			that.cache.grid = $('#memberMainTradeList').ligerGetGridManager();
			$('#memberMainTradeList').css('top', '0px');
		...
		search : function() {
			...
			if (cardNum) {
				parms['requestParam.like.cardNo']="%"+cardNum+"%";
			}
			if (tradeType) {
		...
		output : function() {
			...
			var excelUrl = root + '/mainCardTrade/outputMemberTradeList.action';
			if (parmStr != "") {
				excelUrl += "?" + parmStr;
			}
